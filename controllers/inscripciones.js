...
exports.index = async (req, res) => {
  ...
  try {
    ...
    const inscripciones = await Inscription.findAll();
...
exports.store = async (req, res) => {
  try {
    ...
    res.status(500).send({
      message:
        error.message ||
        "No hemos podido  crear el ins , revisa los datos introducidos",
    });
  }
};
exports.show = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      message:
        error.message ||
        "No hemos podido  encontrar el ins que has seleccionado",
    });
  }
};
exports.update = async (req, res) => {
  try {
    ...
    res.status(404).send({
      message:
        error.message ||
        "No hemos podido encontrar el ins que quieres modificar",
    });
  }
};
